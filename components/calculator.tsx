...
const SalaryCalculator = () => {
    ...
    const calculateWorkingDays = useCallback(() => {
        const yearDays = 365;
        const totalLeaves = formData.annualLeave + formData.publicHolidays + (formData.monthlyFreeDays) * 12;
    ...
    const calculateWorkHours = useCallback(() => {
        ...
        return formData.offWorkHour - formData.workHour + formData.commuteHours - formData.breakHours;
    }, [formData.workHour, formData.offWorkHour, formData.commuteHours, formData.breakHours]);
    // 工作时长转化的牛马系数
    ...
    const calculateValue = () => {
        ...
        // 计算系数
        const environmentFactor = parseFloat(formData.heterogeneity) * parseFloat(formData.workEnvironment) * parseFloat(formData.teamwork)
            * getDayWorkHourFactor * getDayCommuteHourFactor;
    ...
    const getValueAssessment = () => {
        if (!formData.monthlySalary) return {text: "请输入月薪", color: "text-gray-500"};
        if (value < 0.6) return {text: "底层牛马", color: "text-gray-500"};
        if (value >= 0.6 && value < 0.8) return {text: "下级牛马", color: "text-red-500"};
        if (value >= 0.8 && value < 1.0) return {text: "普通牛马", color: "text-blue-500"};
        if (value >= 1.0 && value < 1.2) return {text: "上级牛马", color: "text-yellow-500"};
        if (value >= 1.2 && value < 1.4) return {text: "高级牛马", color: "text-green-500"};
                ...
                <div className="p-6 space-y-8">
                    ...
                    <div className="space-y-6">
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700">月薪（元）</label>
                                <input
                                    type="number"
                                    min="0"
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700">每年发薪月数</label>
                                <input
                                    type="number"
                                    min="1"
                                    step="0.5"
                                    value={formData.yearSalaryNum}
                                    onChange={(e) => handleInputChange('yearSalaryNum', e.target.value)}
                                    placeholder="每年发薪月数"
                                    className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                />
                            </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700">月休天数</label>
                                <input
                                    min="0"
                                    step="0.5"
                                    type="number"
                                    value={formData.monthlyFreeDays}
                                    onChange={(e) => handleInputChange('monthlyFreeDays', e.target.value)}
                                    className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                />
                            </div>
                            <div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => handleInputChange('offWorkHour', e.target.value)}
                                    className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                />
                            </div>
                            <div>
            ...
            <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-6 shadow-inner">
                <div className="grid grid-cols-3 gap-8">
                    <div>
                        ...
                        <div className="text-2xl font-semibold mt-1">{calculateWorkingDays().valueOf()}天</div>
                    </div>
                    <div>
